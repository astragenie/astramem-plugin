 ...
 */
export function astraMemoryDir() {
...
export function tokensFilePath(env) {
 ...
 *
 * @param {string} env  — e.g. "prod", "local", "staging"
 * @returns {{ apiUrl: string, env: string }}
 * @throws  with .exitCode = 4 when profile is missing
 */
export async function resolveProfile(env) {
 ...
 */
export async function resolveToken(env, workspaceId) {
 ...
 * @param {string} env
 * @param {string} workspaceId
 * @param {{ apiKey: string, label: string, tenantId: string, repoPath: string, pairedAt: string }} entry
 */
export async function writeToken(env, workspaceId, entry) {
